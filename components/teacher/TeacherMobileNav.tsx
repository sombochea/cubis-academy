...
import { Menu, X, LayoutDashboard, BookOpen, Users } from 'lucide-react';
import { Trans } from '@lingui/react/macro';
import { Button } from '@/components/ui/button';

export function TeacherMobileNav({ locale }: { locale: string }) {
  const [isOpen, setIsOpen] = useState(false);
  const pathname = usePathname();

  const navItems = [
    { href: `/${locale}/teacher`, label: 'Dashboard', icon: LayoutDashboard, exact: true },
    { href: `/${locale}/teacher/courses`, label: 'My Courses', icon: BookOpen },
    { href: `/${locale}/teacher/students`, label: 'Students', icon: Users },
  ];
